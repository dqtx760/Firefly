...
tags:
  - "Clipings"
---
![图像](https://pbs.twimg.com/media/HJopSnWaIAAMZOZ?format=jpg&name=large)

> Claude Code 装好了，Skills 配了，MCP 接了。AI 还是动不动胡说八道，质量靠抽卡。这一篇讲清楚：**为什么会这样，怎么管住它**。

# 第 1 章 你装了一堆东西，AI 还是瞎搞，为什么

## 1.1 一个真实的场景

你按部就班把工具都装齐了：

- 装了 Claude Code，配好了国产模型。
- 写好了 CLAUDE.md，告诉它你是谁、你的偏好、你的红线。
- 装了 5 个常用 Skills，公众号发布、配图、改写都齐了。
- 接了 3 个 MCP：Tavily 搜索、飞书、小红书。
...
- 让它整理 10 个文件，整理了 3 个就开始重复检查同一个。
- 让它调用 MCP 发小红书，它说"已发送成功"，你打开 App 一看，根本没发。
...
**这是 99% 小白的第一反应。也是 99% 小白浪费钱的开始。**

## 1.2 换模型解决不了这件事

2026 年 2 月有一个被疯传的实验：

- 同一个模型，编程能力测试通过率原来是 **6.7%**。
- 不换模型，只改"外面那一层"——**通过率飙到 68.3%**。

**模型一个字节都没动。**

类似的实验后来又有几家公司复现过——LangChain、OpenAI、Anthropic 各自的内部团队都得到了类似的结论：

> 当模型够强之后，决定 AI 干活靠不靠谱的，已经不是模型本身。是模型**外面那套东西**。
...
- **Model**：模型本身，就是 Claude、GPT、GLM 这些。
- **Harness**：模型之外的一切——约束、后置、记忆、工具、权限、监控。

你平时用的那些工具，**都是 Harness 的零件**：

**但这些只是 Harness 的一半，前置控制那一半**。
...
第五个阶段，他起的名字叫 **Engineer the Harness（挽具工程）**。
...
![图像](https://pbs.twimg.com/media/HJolqhcbsAAKY7L?format=jpg&name=large)
...
理解了这一点，以后你看到任何一个新的 Harness 组件，都能问出关键问题：**它在补哪个短板？**

补不出短板的组件 → 是过度工程化，**别装**。

## 1.6 AI 工程的三个阶段

Harness 不是凭空出现的概念。它是 AI 应用工程的**第三个阶段**。

![图像](https://pbs.twimg.com/media/HJolrAqb0AArkUA?format=jpg&name=large)

为什么 2026 年这件事突然变重要？

因为模型本身能力已经够强。**Prompt 写得多花哨、Context 给得多准，带来的提升越来越小**。
...
![图像](https://pbs.twimg.com/media/HJolrhmbIAAucvw?format=jpg&name=large)

最后一条尤其重要。

很多 Harness 文章是写给企业工程师看的——讲 CI 流水线、Linter 规则、企业级监控系统。

**但 Harness 的本质思想，跟你个人小白完全相通**：

![图像](https://pbs.twimg.com/media/HJolsCpbAAAdkUx?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJolsirbIAEgjdy?format=jpg&name=large)

**全是"后置"那一半缺位**。

你之前配了大量"前置"——CLAUDE.md、Skills、MCP，告诉 AI 该怎么做。

但你没配任何"后置"——没有人在 AI 做完后**强制检查**它做得对不对。
...
![图像](https://pbs.twimg.com/media/HJolpZDaAAANjPA?format=jpg&name=large)

如果你 Claude Code 用过一段时间，前置这一半你**基本就掌握了 80%**。
...
![图像](https://pbs.twimg.com/media/HJol0MCaoAAFRCr?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol00JbcAAkddn?format=jpg&name=large)
...
这就是前置和后置的**本质区别**：**一个靠 AI 自觉，一个靠系统强制**。
...
## 4.1 95% 的人 CLAUDE.md 写错了

打开你自己的 CLAUDE.md 看一眼。是不是长这样：
...
**这就是典型的"愿望清单"**——一堆"我希望 AI 做到的事"。

愿望清单的问题在哪？

![图像](https://pbs.twimg.com/media/HJol1TFaMAAoYsF?format=jpg&name=large)

结果就是：你的 CLAUDE.md 越写越长，AI 翻车的次数一点没少。

## 4.2 升级思路：从愿望清单到失败日志

Mitchell Hashimoto（HashiCorp 联合创始人）有一句话被 Harness 圈子奉为圣经：
...
![图像](https://pbs.twimg.com/media/HJol100aEAAfYC9?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol2YjaQAAsBGq?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol21XbgAAF53T?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJolp8AbAAAi9GK?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol_PJbwAAT9B0?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol_PabUAAUEi6?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJomANTaMAAkL6c?format=jpg&name=large)
...
![图像](https://pbs.twimg.com/media/HJol-2gbQAAE0ZU?format=jpg&name=large)
