...
<!-- 配图：终端运行 feishu-acp 启动成功的截图，warm 风格，16:9 -->

### 常见问题

**Q：bun add -g 装不上，一直卡着怎么办？**

国内网络访问 GitHub 确实慢。可以试试挂代理，或者多等一会儿。实在不行换个时间段再试。

**Q：微信扫码之后没反应？**

检查终端是不是支持二维码显示。有些终端（比如 Windows 自带的 cmd）显示不了二维码，建议用 Windows Terminal 或 iTerm2。

**Q：飞书发了消息没收到回复？**

先检查应用有没有发布成功，权限有没有配全。常见的有两个坑：一是忘记加 `im.message.receive_v1` 事件，二是忘记在版本管理里点"发布"。这两个坑我当时都踩过。

**Q：wechat-acp 和 feishu-acp 可以同时跑吗？**

可以。两个是独立的进程，互不冲突。一个终端跑微信，另一个跑飞书，俩都能用。

### 总结

- ACP 模式一条命令启动，比 Channel 模式省事太多
- 微信最简单，装完扫码就完事
- 飞书需要配应用，配一次以后也不用管了
- 支持 Claude、Gemini、Qwen 等多种 Agent 后端
