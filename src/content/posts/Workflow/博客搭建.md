...
**现象**：点击导航栏链接（首页、归档等）时，文字会向右抖动/拉扯，像是位移了一样。

**排查过程**：花了很长时间排查代码，尝试了移除 `onload-animation`、`scale-animation`、`active:scale-95` 等动画类，修改 Swup 过渡动画 CSS，添加 `contain: layout` 等多种方案，本地始终没有解决。

**原因**：最终发现是**浏览器本地缓存**导致的。推送到 GitHub 构建部署后，访问线上网站时完全没有这个问题。

**经验教训**：

1. **先对比线上版本**：遇到视觉问题时，先检查 GitHub 部署后的线上版本是否也有同样的问题。如果线上正常，大概率是本地缓存。
2. **清除缓存再测试**：`Ctrl+Shift+Delete` 清除浏览器缓存后重新加载，很多"诡异"问题会自动消失。
3. **不要在缓存问题上浪费时间**：如果改了很多代码都没效果，停下来想想是不是根本不是代码的问题。
