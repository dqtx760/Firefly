...
function createInterface(posts, initialQuery = "") {
	...
	function render() {
		...
		const lines = [
			`${ANSI.bold}${ANSI.cyan}DQTX BLOG SEARCH${ANSI.reset}  ${ANSI.dim}大强博客 · blog.dqtx.cc${ANSI.reset}`,
...
async function main() {
	...
	if (!process.stdin.isTTY || !process.stdout.isTTY) {
		console.error("dqtx 需要在交互式终端中运行。请直接打开终端后输入 dqtx。");
		process.exitCode = 1;
		return;
	}

	process.stdout.write("正在获取博客文章索引…\n");
	try {
		const posts = await loadPosts();
		createInterface(posts, args.join(" "));
	} catch (error) {
